...
public class ClickGuiScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        if (bindingModule != null) {
            ...
            context.drawTextWithShadow(textRenderer, Text.literal(txt), width / 2 - tw / 2, hintY + 2, theme.primary.getRGB());
        }
    ...
    private void renderHeader(DrawContext context, int x, int y, int w, GUITheme theme) {
        ...
        context.drawTextWithShadow(textRenderer, Text.literal(title), x + 14, y + 16, titleColor);

        int nickW = textRenderer.getWidth(nickname);
        context.drawTextWithShadow(textRenderer, Text.literal(nickname), x + (w - nickW) / 2, y + 16, theme.text.getRGB());

        int timeW = textRenderer.getWidth(time);
        context.drawTextWithShadow(textRenderer, Text.literal(time), x + w - timeW - 14, y + 16, theme.primary.getRGB());

        String sub = "v1.21.11  |  Theme: " + theme.name;
        context.drawTextWithShadow(textRenderer, Text.literal(sub), x + 14, y + 26, theme.textSecondary.getRGB());
    }
    ...
    private void renderCategoryBar(DrawContext context, int x, int y, int w, int mouseX, int mouseY, GUITheme theme) {
        ...
        for (int i = 0; i < cats.length; i++) {
            ...
            context.drawTextWithShadow(textRenderer, Text.literal(label), tx + (tabW - 4 - lw) / 2, y + 6, textColor);
        }
    }
    ...
    private void renderModuleList(DrawContext context, int x, int y, int w, int h, int mouseX, int mouseY, GUITheme theme) {
        ...
        context.drawTextWithShadow(textRenderer, Text.literal("Modules (" + modules.size() + ")"), x, y - 2, theme.textSecondary.getRGB());
    ...
    private void renderModuleDetails(DrawContext context, int x, int y, int w, int h, int mouseX, int mouseY, GUITheme theme) {
        if (selectedModule == null) {
            ...
            context.drawTextWithShadow(textRenderer, Text.literal(placeholder), x + (w - tw) / 2, y + h / 3, theme.textSecondary.getRGB());
            return;
        }

        context.drawTextWithShadow(textRenderer, Text.literal(selectedModule.getName()), x, y, theme.primary.getRGB());
        context.drawTextWithShadow(textRenderer, Text.literal(selectedModule.getDescription()), x, y + 12, theme.textSecondary.getRGB());
    ...
    private void renderSettingItem(DrawContext context, int x, int y, int w, Setting<?> setting, int mouseX, int mouseY, GUITheme theme) {
        context.drawTextWithShadow(textRenderer, Text.literal(setting.getName()), x, y + 2, theme.text.getRGB());
        ...
        if (setting instanceof BooleanSetting bs) {
            ...
            context.drawTextWithShadow(textRenderer, Text.literal(mode), x + w - modeW - 4, y + 2, theme.primary.getRGB());
            ...
            context.drawTextWithShadow(textRenderer, Text.literal(val), x + w - valW - 4, y + 2, theme.textSecondary.getRGB());
        }
    }
    ...
    private void renderSpotifyTab(DrawContext context, int x, int y, int w, int h, int mouseX, int mouseY, GUITheme theme) {
        ...
        context.drawTextWithShadow(textRenderer, Text.literal("Spotify Player (Pulse Integration)"), x + 15, y + 15, theme.primary.getRGB());

        SpotifyManager spotify = SpotifyManager.getInstance();
        String track = spotify.getCurrentTrack();
        context.drawTextWithShadow(textRenderer, Text.literal("Track: " + track), x + 15, y + 32, theme.text.getRGB());
